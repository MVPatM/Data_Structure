...
int main(void) {
    ...
    // open the file
    ifstream fin;
    fin.open("Q3_5.txt"); // ""안에 file 이름 넣기
...
void MergeSort(int from, int to) {
    ...
    int start = from, left = from, right = mid + 1;
    
    while (left <= mid && right <= to) {
        if (arr[left] > arr[right]) {
            varArr[start] = arr[right];
            right++;
        }
        else {
            varArr[start] = arr[left];
            left++;
        }
